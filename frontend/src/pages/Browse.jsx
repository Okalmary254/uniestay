...
export default function Browse() {
  ...
  const selectUni = (uni) => { setUniFilter(uni); setUniInput(uni); setShowDrop(false) }
  const clearUni  = ()    => { setUniFilter('');  setUniInput('') }

  const fetchProperties = async () => {
    setLoading(true)
    try {
      const params = {}
      if (uniFilter)  params.search        = uniFilter
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 md:py-6">
        ...
        <div className="card mb-4 md:mb-6">
          ...
          <form onSubmit={handleSearch} className="flex flex-col gap-2">
            {/* Row 1 — text search + uni dropdown */}
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <div className="relative flex-1" ref={dropRef}>
                <div className="relative">
                  ...
                  />
                  {uniInput ? (
                    <button type="button" onClick={clearUni}
                      className="absolute right-2 top-1/2 -translate-y-1/2
                                 text-gray-400 hover:text-gray-600 text-lg leading-none">×</button>
                  ) : (
                    <span className="absolute right-2 top-1/2 -translate-y-1/2
                                     text-gray-400 pointer-events-none text-xs">▾</span>
                  )}
                </div>
                {showDrop && (
                  <ul className="absolute z-20 top-full left-0 right-0 mt-1 bg-white
                                 border border-gray-200 rounded-lg shadow-lg
                                 max-h-48 overflow-y-auto">
                    <li onClick={() => { clearUni(); setShowDrop(false) }}
                      className="px-3 py-2 text-sm text-gray-400 hover:bg-gray-50
                                 cursor-pointer border-b border-gray-100">
                      Any university
                    </li>
                    {filteredUnis.map(uni => (
                      <li key={uni} onClick={() => selectUni(uni)}
              ...
              {typeFilter && (
                <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1
                  ...
                  <button onClick={() => setTypeFilter('')}
                    className="hover:text-teal-900 font-medium">×</button>
                </span>
              )}
              {maxRent && (
                <span className="inline-flex items-center gap-1 text-xs px-2.5 py-1
        ...
        {/* Results grid — 1 col mobile, 2 sm, 3 md, 4 lg */}
        {loading ? (
          <Spinner />
        ) : properties.length === 0 ? (
